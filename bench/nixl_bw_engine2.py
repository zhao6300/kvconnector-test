...
def main():
    args = parse_args()
    torch.set_default_device("cuda:0" if args.mode == "target" else "cuda:0")
    tensor = torch.ones(args.size, dtype=torch.uint8) if args.mode == "target" else torch.zeros(args.size, dtype=torch.uint8)

    config = nixl_agent_config(True, True, args.port if args.mode == "target" else 0, backends=[])
    agent = nixl_agent(args.mode, config)
    agent.create_backend("UCX", {"engine_config": "TLS=cuda_copy,tcp"})
    ...
    print("# NIXL UCX P2P READ (world=2)")
